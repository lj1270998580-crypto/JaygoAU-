# Jaygo AU · 火山引擎豆包语音工作台

一个桌面端应用，用于**可视化上传音频 → 复刻专属音色 → 选择音色 → 输入文本合成语音 → 上传视音频转录成文字**。全部云端能力基于火山引擎「豆包语音」声音复刻 / 语音合成 / 录音文件识别 API，用户只需在设置中填入 API Key 即可使用。界面采用 **简洁风格**（类似 Notion/Linear 的白底细边框设计）。

> **跨平台说明**：当前仅提供 Windows 安装包，但项目基于 Electron 构建，理论上支持 macOS 和 Linux。如需其他平台版本，可自行修改 `package.json` 中的构建配置并执行 `npm run dist`。

## ✨ 功能

- **设置**：填入 `X-Api-Key`（本机 `safeStorage` 加密存储，不进前端代码 / DevTools）；配置默认合成参数、输出目录、阿里云 OSS（转录暂存）、火山 AK/SK（余额查询）。
...
- **视音频转录**：上传音频或视频（mp4/mov/...），软件自动提取音轨（视频）或直传音频，经阿里云 OSS 临时托管（任务结束即删）后调用火山引擎**录音文件识别 2.0**，输出**带标点、可分段**的文本，可选「说话人分离」。
- **音频库**：本次会话内合成结果集中管理，支持**在线试听**与**另存为下载**。
- **账户余额**：软件左下角**实时显示**火山引擎账户可用余额（每 60 秒刷新，需配置 AK/SK），鼠标悬停可看现金 / 欠费 / 冻结明细。
...
│  • IPC handlers：复刻 / 查询 / 合成(NDJSON 流式) /            │
│    视音频转录(oss 暂存 + ffmpeg 提取) / 余额查询 / 文件读写   │
│  • 所有 HTTPS 请求在此发起（天然绕过浏览器 CORS）             │
└───────────────┬──────────────────────────────────────────┘
                │ HTTPS (X-Api-Key / AK-SK 签名)
┌───────────────▼──────────────────────────────────────────┐
│  火山引擎 openspeech.bytedance.com                           │
│  /api/v3/tts/voice_clone  ·  /get_voice  ·  /unidirectional │
│  /api/v3/auc/bigmodel/submit · /query（录音文件识别 2.0）    │
│  billing.volcengineapi.com（QueryBalanceAcct 余额查询）      │
└──────────────────────────────────────────────────────────┘
```
...
以下为火山引擎「豆包语音」各能力的**官方刊例价**（价格可能调整，以官网为准）。软件「设置 → 官方价格说明」内也内置了实时对照表。

| 功能 | 计费单位 | 按量付费 | 资源包（参考） |
| --- | --- | --- | --- |
| 语音合成 2.0 (`seed-tts-2.0`) | 元 / 万字符 | ¥5 / 万字符 | 约 ¥2.8 / 万字符（10 万字包约 ¥28） |
| 声音复刻 2.0（推理） | 元 / 万字符 | ¥8 / 万字符 | 约 ¥2.8 / 万字符 |
| 录音文件识别 2.0（转录） | 元 / 小时 | ¥0.8 / 小时 | 约 ¥0.9 / 小时（1000 小时包约 ¥900） |
| 流式语音识别 2.0 | 元 / 小时 | ¥1.0 / 小时 | 以官网资源包为准 |

**说明**
- **声音复刻额外费用**：复刻本身免费，但每个音色首次调用合成接口即视为「转正」，会收取一次性**音色槽位费**；后续按合成字符数计费。
- **字符数口径**：合成按实际合成文本字符数计费；转录按音频**时长（小时）**计费，与文本长度无关。
- **省钱建议**：长期高频使用优先购买**资源包**，用量小则用按量付费即可。余额可在软件左下角实时查看。
...
└─ src/
   ...
   ├─ lib/{ipc,format,pricing}.ts
   └─ components/{Settings,Clone,Voices,Synthesize,Transcribe,Library,AudioPlayer}.tsx
```

## 已实现的增强

- **视音频转录（录音文件识别 2.0）**：上传音频 / 视频 → 视频自动 ffmpeg 提取音轨 → 阿里云 OSS 临时托管（任务结束即删）→ 调用火山引擎识别 2.0，输出带标点、可分段的文本，支持说话人分离。
- **账户余额实时显示**：左下角每 60 秒刷新火山引擎可用余额（需配置 AK/SK，走 AK/SK V4 签名），悬停看现金 / 欠费 / 冻结明细。
- **官方价格说明内置**：设置页内置四能力刊例价对照表，让用户清楚每次调用的消费口径。
