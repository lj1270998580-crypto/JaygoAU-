...
/** 硬性推销标记：出现即判定该句为推销/带货内容（v0.7.24 扩充电商带货、课程促销、赠品诱导与价格优惠拦截） */
export const SALES_PITCH_HARD_REGEX =
  /(购买课程|点击下方|小黄车|购物车|下单|拍下|去买|买它|橱窗|私信|粉丝群|粉丝团|福利价|秒杀|原价.{0,12}(现价|现在|只要)|现价|到手价|券后|特惠价|限时价|打折|立减|左下角|下方链接|链接在|购买链接|课程链接|领\s*取?\s*资料|免费\s*领|免费\s*送|赠送|还加赠|送.{0,8}份(协议|合同|资料|模板|工具包|课件)|价值\s*[>＞大于超过]?\s*[0-9]+元?|扣\s*个?\s*[0-9]|评论区.{0,6}(扣|回复|留言)|关注直播间|赶紧抢|赶紧上车|抢购|手慢无|优惠券|扫码|加.{0,6}(微信|群)|入群|三晚直播|纯干货.*名额|限额|名额有限|仅限.{0,8}名|前\s*[0-9]+\s*名|前\s*[0-9]+\s*位|先到先得|报名|内部课|私房课|训练营|股权.*元|合规.*元|[0-9]+元.{0,8}课程|课程.{0,8}[0-9]+元|今天拍下|只要\s*[0-9]+元?|最后\s*[0-9]+\s*个名额|仅剩\s*[0-9]+|我在.*等你)/;

/** 软性标记：招呼/互动引导，单独出现不足以判定整句是推销 */
export const SALES_PITCH_SOFT_REGEX =
  /(关注我|点赞|收藏|转发|三连|家人们|感谢大家观看|感谢观看|需要的朋友|不迷路|限时|福利|抓紧时间|倒计时)/;
