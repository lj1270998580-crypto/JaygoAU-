...
export const LAYOUTS: Record<string, LayoutSpec> = {
  ...
  // —— v0.7.24 补齐商汤官方 88 全集版式 ——
  'asymmetry': {
    id: 'asymmetry',
    label: '非对称构图',
    category: 'comparison',
    desc: '左右极端失衡，通过视觉重量达成动态平衡',
    prompt: '画面采用刻意的非对称构图，一侧为大体量视觉主体或密集信息，另一侧留白或简短文字，利用视觉重量差制造动态张力',
  },
  'breaking-the-grid': {
    id: 'breaking-the-grid',
    label: '破格越界',
    category: 'focus',
    desc: '主体冲破卡片或网格边界，营造破框立体感',
    prompt: '画面核心元素或主体突破卡片或边框的束缚向外延伸破格，打破网格限制，呈现强烈的立体感与动态视觉冲击',
  },
  'chapter-layout': {
    id: 'chapter-layout',
    label: '分卷章节',
    category: 'hierarchy',
    desc: '书籍分卷式结构，板块之间有清晰的章回视觉分割',
    prompt: '画面呈现清晰的书籍分卷章节结构，各板块之间有典雅明确的章节标记与分隔带，条理井然',
  },
  'character-guide': {
    id: 'character-guide',
    label: '人物向导',
    category: 'narrative',
    desc: '角落或边缘穿插小人物，指引视觉视线',
    prompt: '画面边缘或角落安排一位生动的人物角色作为视觉向导，以手势或视线引导观众看向核心内容区域',
  },
  'collage-glitch': {
    id: 'collage-glitch',
    label: '故障拼贴',
    category: 'metaphor',
    desc: '撕纸边缘与错位拼贴，带微故障艺术感',
    prompt: '画面采用波普剪纸撕边与故障艺术拼贴风格，不同材质切片与色块错位交织，呈现前卫张力',
  },
  'comparison-matrix': {
    id: 'comparison-matrix',
    label: '全景对比矩阵',
    category: 'comparison',
    desc: '网格多行多列，全方位横纵向参数对比',
    prompt: '画面分为多行多列的规整网格矩阵，横向为对比维度，纵向为对比对象，单元格内用清晰的标记或数值展示全景对比',
  },
  'disrupted-flow': {
    id: 'disrupted-flow',
    label: '破序流动',
    category: 'narrative',
    desc: '打破传统阅读流，通过交错排布引导探索',
    prompt: '画面打破常规从左到右阅读顺位，通过多向交错的视线引导线与模块穿插，形成富有探索欲的动态节奏',
  },
  'edge-tension': {
    id: 'edge-tension',
    label: '边缘张力',
    category: 'comparison',
    desc: '主体紧贴画布边缘甚至局部裁切，制造紧迫感',
    prompt: '核心主体紧贴画面边缘排布甚至局部延伸出画布，利用边缘挤压效应制造强烈的视觉张力与悬念感',
  },
  'f-pattern': {
    id: 'f-pattern',
    label: 'F型视觉流',
    category: 'data',
    desc: '经典眼动F型，顶部横扫、左侧纵深阅读',
    prompt: '版面严格遵循经典 F 型视觉眼动轨迹，顶部横向强标题，左侧纵向关键锚点，次级信息向右自然延展',
  },
  'flashback': {
    id: 'flashback',
    label: '闪回倒叙',
    category: 'narrative',
    desc: '主画面嵌入时空小窗，展示过去或未来切片',
    prompt: '当前核心场景中嵌入若干时空闪回的独立画框，画框内展示过去因果或未来推演，形成跨时空叙事对照',
  },
  'frame-composition': {
    id: 'frame-composition',
    label: '画中画框',
    category: 'focus',
    desc: '前景装饰门框或负空间构筑框景，聚焦主体',
    prompt: '利用前景几何框、窗棂或负空间环抱构成画中画框景，将观众视线牢牢锁定在框内的视觉核心',
  },
  'full-illustration': {
    id: 'full-illustration',
    label: '通栏全幅画卷',
    category: 'narrative',
    desc: '大面积完整场景插画主导，文字自然融入',
    prompt: '全景通栏插画主导整个画面空间，空间层次深邃丰富，说明文字极简并自然融入场景留白区域',
  },
  'gallery-style': {
    id: 'gallery-style',
    label: '画廊陈列',
    category: 'focus',
    desc: '大面积纯净留白，如同美术馆展品独立陈列',
    prompt: '大面积艺术展厅式纯白留白空间，展品或图表如美术馆典藏般独立悬浮陈列，优雅静谧',
  },
  'hidden-details': {
    id: 'hidden-details',
    label: '微距寻宝解构',
    category: 'metaphor',
    desc: '宏观大场景中隐藏精致细节，供显微细看',
    prompt: '宏观丰富的大场景中巧妙隐藏了若干精致的次级线索与微距小细节，引人入胜',
  },
  'isometric-tech-stack': {
    id: 'isometric-tech-stack',
    label: '等距技术栈',
    category: 'hierarchy',
    desc: '2.5D 等距视角，由底到顶立体分层堆叠',
    prompt: '采用 2.5D 等轴测（Isometric）三维视角，自底向上立体分层堆叠各系统层级，层间微光悬浮',
  },
  'left-text-right-image': {
    ...
    category: 'narrative',
    desc: '左侧文字阐述、右侧大图印证',
    prompt: '画面分为左右两半，左侧为工整的信息与观点陈述，右侧为生动的视觉主体大图，图文呼应',
  },
  'top-image-bottom-text': {
    id: 'top-image-bottom-text',
    label: '上图下文',
    category: 'narrative',
    desc: '上方视觉冲击大图、下方提炼要点',
    prompt: '画面呈经典上下两段式，上方占据较大面积的强视觉冲击插画，下方收拢为精炼的要点结论',
  },
  'luxury-layout': {
    id: 'luxury-layout',
    label: '极简轻奢',
    category: 'focus',
    desc: '黑金/深蓝雅致配色，居中对称典雅版式',
    prompt: '黑白金或深靛蓝的雅致配色，居中对称的典雅构图，克制微小的几何点缀与充裕呼吸感',
  },
  'maximalism': {
    id: 'maximalism',
    label: '极繁饱满',
    category: 'data',
    desc: '高密度信息阵列，细节丰富毫无冷场',
    prompt: '极繁主义布局，画面空间被丰富精致的数据标签、指标与细节饱满填充，信息密度极高但层次井然',
  },
  'multi-directional': {
    ...
    category: 'relation',
    desc: '核心向上下左右多维度发散连接',
    prompt: '中心节点向四周多维度呈放射状发散连接，网状脉络清晰贯通，展示复杂的多维关联',
  },
  'nine-grid': {
    id: 'nine-grid',
    label: '九宫格矩阵',
    category: 'data',
    desc: '经典 3×3 规整九宫格，各单元均衡',
    prompt: '画布均等划分为 3×3 经典九宫格矩阵，9 个单元格结构工整统一，中心单元或为核心视线聚焦点',
  },
  'nonlinear-path': {
    id: 'nonlinear-path',
    label: '非线性漫游',
    category: 'metaphor',
    desc: '跳跃式形状与路径连接，自由探索',
    prompt: '节点排布打破死板直线，沿自然流线或跳跃几何路径曲折串联，引导观众自由漫游探索',
  },
  'overlapping': {
    id: 'overlapping',
    label: '错落层叠',
    category: 'hierarchy',
    desc: '多层卡片前后错落叠放，带来景深层次',
    prompt: '多个矩形卡片与图文色块前后层叠交错摆放，利用半透明遮挡与柔和投影形成丰富的景深空间感',
  },
  'random-scatter': {
    id: 'random-scatter',
    label: '自由散点',
    category: 'metaphor',
    desc: '意象元素散落分布，如同头脑风暴星图',
    prompt: '元素看似随机但疏密有致地散落分布于画布各处，如星图般相互呼应，极具灵动发散感',
  },
  'speech-bubbles': {
    id: 'speech-bubbles',
    label: '气泡对话',
    category: 'relation',
    desc: '漫画式对话框或问答交替气泡',
    prompt: '利用大小错落的圆角对话气泡在画面中展开交锋与呼应，带有生动的提问与解答交互感',
  },
  'spiral-vortex': {
    id: 'spiral-vortex',
    label: '螺旋涡流',
    category: 'process',
    desc: '对数螺旋向心汇聚，由外向内层层深入',
    prompt: '元素沿优美的阿基米德螺旋线或涡流轨迹由外向内向心汇聚，表达由浅入深、层层收敛的动态',
  },
  'text-wrap': {
    id: 'text-wrap',
    label: '图文包络',
    category: 'narrative',
    desc: '文字沿着主体轮廓紧密环绕',
    prompt: '排版文字沿着中央主体的有机轮廓边缘紧密环绕贴合，图文交融一体，富有高端杂志版式质感',
  },
  'three-tier': {
    id: 'three-tier',
    label: '三层渐进',
    category: 'hierarchy',
    desc: '上中下明确三层结构，基础-核心-顶层',
    prompt: '画面自底向上划分为清晰的底层基础、中层支柱与顶层战略三层阶梯结构，层次分明',
  },
  'tree-branching': {
    id: 'tree-branching',
    label: '树状分支',
    category: 'hierarchy',
    desc: '主干发散到枝叶，展示分类与派生',
    prompt: '以清晰的主干向外开枝散叶的分支拓扑结构排布，展示父级到子级的分类派生与逻辑树',
  },
  'visual-first': {
    id: 'visual-first',
    label: '视觉大图优先',
    category: 'focus',
    desc: '超大面积主视觉占据 80%，极简配字',
    prompt: '巨大冲击力的单一主视觉占据全画幅 80% 空间，说明文字高度克制收敛于角落，第一眼即震撼',
  },
  'wave-path': {
    id: 'wave-path',
    label: '波浪流线',
    category: 'process',
    desc: '正弦波形路径，表达周期与波动起伏',
    prompt: '一条优雅平滑的正弦波浪曲线起伏穿行于画面，各节点依波峰波谷错落排布，形象表达周期与起伏',
  },
};
