...
export const TalkTimeline: React.FC<TalkTimelineProps> = ({
  ...
  // 计算轨道总像素宽度
  const baseWidth = Math.max(viewportWidth - 120, 800);
  const trackWidth = Math.round(baseWidth * zoomLevel);
  const pxPerSec = trackWidth / dur;

  // 极速声学包络波形生成（毫秒级生成，零内存占用，彻底杜绝主线程卡死）
  const waveformPeaks = useMemo(() => {
    const sampleCount = 500;
    const peaks: number[] = new Array(sampleCount);

    for (let i = 0; i < sampleCount; i++) {
      const t = (i / sampleCount) * dur;
      // 检查当前时刻是否在被标记为静音气口或未说话段落中
      const inSilence = segments.some(
        (s) => (s.isDeleted || s.deleteReason === 'silence') && t >= s.startTime && t <= s.endTime
      );

      if (inSilence) {
        peaks[i] = 0.04 + (Math.sin(i * 3.7) * 0.02);
      } else {
        // 自然人声能量包络曲线
        const base = 0.45 + 0.35 * Math.sin(i * 0.25) * Math.cos(i * 0.13);
        const flutter = (Math.sin(i * 1.8) + Math.cos(i * 2.9)) * 0.12;
        peaks[i] = Math.max(0.12, Math.min(0.95, base + flutter));
      }
    }
    return peaks;
  }, [segments, dur]);
  ...
  const getSnapTime = useCallback(
    (targetTime: number, thresholdPx = 10): number => {
  ...
  // 处理标尺区域 Scrubbing
  const handleTimelineScrub = useCallback((e: MouseEvent | React.MouseEvent) => {
    ...
    onSeek(finalTime);
  }, [dur, pxPerSec, getSnapTime, onSeek]);

  // 启动标尺或播放头指针 Scrubbing
  const startRulerScrubbing = (e: React.MouseEvent) => {
    e.preventDefault();
    draggingRef.current = {
      type: 'scrub',
      startX: e.clientX,
  ...
  // 全局鼠标拖拽移动监听 (使用 requestAnimationFrame 保持极致丝滑)
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      rafRef.current = requestAnimationFrame(() => {
        if (drag.type === 'scrub') {
          ...
          const deltaPx = e.clientX - drag.startX;
          const navContainerWidth = 600;
          const scrollDelta = (deltaPx / navContainerWidth) * trackWidth;
  ...
  }, [dur, pxPerSec, getSnapTime, handleTimelineScrub, onTrimSegment, onUpdateSubtitle, trackWidth]);
  ...
  return (
    <div
      ...
      className="flex flex-col bg-[#0c0d12] border-t border-zinc-800 select-none text-zinc-300 overflow-hidden"
      onWheel={handleWheel}
    >
      {/* 1. 时间轴顶部控制工具栏 */}
      <div className="h-10 px-3 bg-[#13141c] border-b border-zinc-800 flex items-center justify-between gap-3 shrink-0">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            type="button"
            onClick={onRestoreAllSegments}
            className="px-2 py-1 rounded bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-zinc-200 text-[11px] font-medium transition cursor-pointer flex items-center gap-1 border border-zinc-800"
            title="恢复全部切片为保留状态"
          >
      ...
      <div className="flex-1 flex min-h-0 relative overflow-hidden">
        ...
        <div className="w-28 bg-[#111218] border-r border-zinc-800 shrink-0 flex flex-col z-20 text-[10.5px]">
          ...
          {/* V2 贴片轨头 */}
          <div className="h-7 border-b border-zinc-800/80 flex items-center justify-between px-2 text-teal-400 font-medium bg-teal-950/10">
            <div className="flex items-center gap-1">
        ...
        >
          ...
          >
            {/* 时间标尺 Ruler (点击或按住标尺拖动播放头) */}
            <div
              className="h-6 border-b border-zinc-800 bg-[#0e0f14] relative cursor-ew-resize hover:bg-[#151620] transition"
              onMouseDown={startRulerScrubbing}
            >
            ...
            <div className="h-10 border-b border-zinc-800/60 relative bg-amber-950/5">
              {subtitles.map((sub) => {
                ...
                const width = Math.max(16, (sub.endTime - sub.startTime) * pxPerSec);
                ...
                return (
                  <div
                    ...
                    className={`absolute inset-y-1 rounded border transition-all cursor-pointer overflow-hidden flex items-center px-1.5 ${
                      isSelected
                        ? 'bg-amber-500/40 border-amber-400 text-white shadow ring-1 ring-amber-400 z-10'
                  ...
                  >
                    ...
                    <div
                      ...
                      className="absolute left-0 inset-y-0 w-2 bg-amber-400/50 hover:bg-amber-300 cursor-ew-resize opacity-0 hover:opacity-100 z-10"
                    />
                    ...
                    <div
                      ...
                      className="absolute right-0 inset-y-0 w-2 bg-amber-400/50 hover:bg-amber-300 cursor-ew-resize opacity-0 hover:opacity-100 z-10"
                    />
                  </div>
                );
              })}
            ...
            {/* V2 贴片轨道 (上下贴片生效区域) */}
            <div className="h-7 border-b border-zinc-800/60 relative bg-teal-950/5">
              {canvasConfig.topPatch?.enabled && (
                <div
                  style={{ left: 0, width: `${trackWidth}px` }}
                  className="absolute inset-y-0.5 rounded bg-teal-900/20 border border-teal-500/30 flex items-center px-2 text-[10px] text-teal-300 truncate"
                  title="顶部大标题贴片全程展示"
                >
                  <Layers className="w-2.5 h-2.5 mr-1 text-teal-400 shrink-0" />
                  <span className="truncate">
                    顶贴：{canvasConfig.topPatch.text || '未填写标题'}
                  </span>
                </div>
              )}
            </div>
            ...
            {/* V1 视频切片主轨 */}
            <div
              className="h-14 border-b border-zinc-800/60 relative bg-indigo-950/5"
              onClick={(e) => {
                // 点击空白轨道移动播放头
                if (!scrollContainerRef.current) return;
                const rect = scrollContainerRef.current.getBoundingClientRect();
                const clickX = e.clientX - rect.left + scrollContainerRef.current.scrollLeft;
                const rawTime = Math.max(0, Math.min(dur, clickX / pxPerSec));
                onSeek(getSnapTime(rawTime));
              }}
            >
              {segments.map((seg, idx) => {
                const left = seg.startTime * pxPerSec;
                const width = Math.max(10, (seg.endTime - seg.startTime) * pxPerSec);
                ...
                return (
                  <div
                    ...
                    className={`absolute inset-y-1 rounded-md border transition-all cursor-pointer overflow-hidden flex flex-col justify-between p-1 group ${
                      isDel
                        ? 'bg-zinc-900/90 border-dashed border-rose-500/50 text-zinc-500 opacity-60'
                        : isSelected
                        ? 'bg-indigo-600/50 border-indigo-300 text-white shadow-lg ring-2 ring-indigo-400 z-10'
                        : 'bg-gradient-to-r from-indigo-950/90 to-purple-950/80 border-indigo-500/50 text-zinc-200 hover:border-indigo-400'
                    }`}
                    title={
                      isDel
                        ? `[已删除: ${seg.tagLabel || '片段'}] ${seg.text || ''} (${(seg.endTime - seg.startTime).toFixed(2)}s) 双击恢复`
                        : `[#${idx + 1}] ${seg.text || ''} (${(seg.endTime - seg.startTime).toFixed(2)}s) 双击标记删除`
                    }
                  >
                    {/* 左侧拉伸把手 */}
                    {!isDel && (
                      <div
                        ...
                        className="absolute left-0 inset-y-0 w-2.5 bg-indigo-400/50 hover:bg-indigo-300 cursor-ew-resize opacity-0 group-hover:opacity-100 z-20 flex items-center justify-center"
                      >
                    ...
                    {/* 切片头部信息条 */}
                    <div className="flex items-center justify-between text-[9.5px] leading-tight select-none pointer-events-none">
                    ...
                    {/* 切片文字摘要 */}
                    <div className="text-[10px] truncate select-none leading-none opacity-90 pointer-events-none">
                      {seg.text || '(无声停顿)'}
                    </div>
                    ...
                    {!isDel && (
                      <div
                        ...
                        className="absolute right-0 inset-y-0 w-2.5 bg-indigo-400/50 hover:bg-indigo-300 cursor-ew-resize opacity-0 group-hover:opacity-100 z-20 flex items-center justify-center"
                      >
                        <div className="w-0.5 h-3 bg-white/80 rounded" />
                      </div>
                    )}
            ...
            {/* A1 声学波形轨道 */}
            <div className="h-12 relative bg-sky-950/5 flex items-center overflow-hidden px-1 pointer-events-none">
              <div className="w-full h-8 flex items-center gap-[1px]">
                {waveformPeaks.map((peak, pIdx) => {
                  const barHeight = Math.max(3, Math.round(peak * 32));
                  return (
                    <div
                      key={pIdx}
                      style={{ height: `${barHeight}px` }}
                      className="flex-1 bg-sky-500/40 rounded-full"
                    />
                  );
                })}
              </div>
            ...
            <div
              ...
              className="absolute inset-y-0 w-0 z-40 transform -translate-x-1/2 pointer-events-none"
            >
              {/* 红色顶部指针倒三角手柄 (支持按住拖拽) */}
              <div
                onMouseDown={startRulerScrubbing}
                style={{ clipPath: 'polygon(0% 0%, 100% 0%, 50% 100%)' }}
                className="w-4 h-4 bg-rose-500 shadow-md transform -translate-x-[7px] pointer-events-auto cursor-ew-resize hover:scale-110 transition-transform"
              />
              {/* 贯穿红色竖线 */}
              <div className="w-[1.5px] h-full bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.7)]" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="h-7 px-3 bg-[#0d0e14] border-t border-zinc-800/80 flex items-center justify-between gap-3 shrink-0 select-none">
        <div className="text-[10px] font-mono text-zinc-500 shrink-0">全景概览</div>
