...
export function ChatMessageRenderer({ content, role, isStreaming }: Props) {
  ...
  const renderBlocks = () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 普通段落文本
      nodes.push(
        <p key={`p-${i}`} className="my-1.5 text-[12px] leading-[1.7] text-zinc-800 dark:text-zinc-200">
  ...
  return (
    <div className="select-text font-script-reading space-y-0.5 text-[12px] leading-[1.7]">
      {/* 深度推演思考卡片 */}
